...
pub fn capture(target: &str, file_path: &str) -> bool {
...
fn take_screenshot(item: &GraphicsCaptureItem, file_path: &str) -> Result<()> {
    ...
    let image: ImageBuffer<Rgba<u8>, _> = ImageBuffer::from_raw(item_size.Width as u32, item_size.Height as u32, bits).unwrap();
